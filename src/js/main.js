...
class Shape {
  constructor (x, y, radius, ax, ay, m, color, vx = 1, vy = 1) {
    this.x = x
    this.y = y
    this.r = radius
    this.ax = ax
  ...
  for (let [i, o1] of o.entries()) {  // For each pair of objects...
    for (let [j, o2] of o.entries()) {
      if (i < j) {  // To not do same pair twice
        ...
        let f = (1000 * o1.m * o2.m) / Math.pow(r, 2)  // Compute force for
        // this pair
...
function createBouncingExample () {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    objects.push(
      new Shape(x, y, radius, getRandomInt(-1, 1), getRandomInt(-1, 1),
        radius * 10))
  }
...
function createShape (event, radius = 20, mass = 100) {
  let x = event.pageX - c.offsetLeft
  let y = event.pageY - c.offsetTop

  objects.push(
    new Shape(x, y, radius, getRandomInt(-1, 1), getRandomInt(-1, 1), mass))
}
